...
export const InventoryManager: React.FC<InventoryManagerProps> = ({ clinicDbId = null }) => {
  ...
  const handleClearAllStock = () => {
    if (confirm('Clear all items from this inventory register? You can add your own real materials or reload standard dental templates.')) {
      persistStock([]);
      setLedger([]);
      setLastDeductedItems([]);
      setLastDeductedProcedure(null);
  ...
  return (
    <div className="space-y-4">
      <div
        role="status"
        className="rounded-md border border-slate-200 bg-white px-3 py-1.5 text-[11px] font-medium text-slate-700 flex items-center justify-between shadow-2xs"
      >
        <span>
          Real-time Consumables & Material Ledger · Auto-deducts on clinical completion (RCT, Composite, Surgery).
        </span>
        <span className="text-[10px] text-teal-700 font-semibold font-mono">
          {materials.length} SKUs Active
        </span>
      </div>
      ...
      <div className="surface-card flex flex-wrap items-center justify-between gap-3 p-4">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="tactile-btn flex items-center space-x-1.5 rounded border border-rose-200 bg-rose-50 px-3 py-1.5 text-xs font-semibold text-rose-700 hover:bg-rose-100"
            title="Clear all stock items"
          >
          ...
          >
            ...
            <span>{isLive ? 'Load Starter SKUs' : 'Reset Demo Stock'}</span>
          </button>
        </div>
      </div>
